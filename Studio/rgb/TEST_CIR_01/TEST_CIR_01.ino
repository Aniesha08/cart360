...
/* ANALOG TRI INPUT @ A0 */
#define TRI         0
#define MODE_STATES 4
#define NOTES       16

int modeIndicator = 0;
int playedNotes = 0; // When in rec mode

void ledThatDebug(int p){
  digitalWrite(THIS_LED, HIGH);
  delay(p);
  digitalWrite(THIS_LED, LOW);
  delay(p);
}
...
void determineModality(){
  if(digitalRead(MODE_BUTTON) == HIGH){
   
    if(modeIndicator < MODE_STATES) {
      delay(500);
      modeIndicator++;
...
void reset_my_machine(){
  modeIndicator = 0;
  playedNotes =0;}
  
void play_my_machine(){
  Serial.println(analogRead(TRI));
  if((analogRead(TRI) < 11) && (analogRead(TRI) < 300))  voice.say(spDANGER);
  if((analogRead(TRI) < 347) && (analogRead(TRI) < 503)) voice.say(spALERT);
  if(analogRead(TRI) < 523) voice.say(spFIRE);
  //tone(PIEZO, analogRead(TRI), 1000);
  //delay(1000);
 }

void selectNextModality(){
  if (modeIndicator == 0){
    reset_my_machine();
  ...
  else if (modeIndicator == 4){
    //randomize_my_machine();
    //Serial.println("randomize mode");
  }
}
...
void setRGB(int m){
  ...
  switch(m){
   ...
   case 4:
    ...
    analogWrite(RGB_G,96);
    analogWrite(RGB_R,196);
    break;
  }
